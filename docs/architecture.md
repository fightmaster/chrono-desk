...
2. **Reuse one versioned timing-core, not a copied processor.** The immutable
   `timing-core v0.2.0` release owns `legacy-once-v1` checkpoint eligibility,
   `member-time-v1`, `result-outcome-v1` and deterministic `ranking-v2`. Chrono Desk keeps the
...
- **Private module provenance.** Release workflows fetch the immutable
  `timing-core v0.2.0` tag with the read-only `TIMING_CORE_READ_TOKEN` GitHub
