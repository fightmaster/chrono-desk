...
2. **Reuse one versioned timing-core, not a copied processor.** The immutable
   `timing-core v0.6.0` release owns `legacy-once-v1` checkpoint eligibility,
   `member-time-v1`, `result-outcome-v2`, deterministic `ranking-v2`, age calculation and the
...
- **Private module provenance.** Release workflows fetch the immutable
  `timing-core v0.6.0` tag with the read-only `TIMING_CORE_READ_TOKEN` GitHub
